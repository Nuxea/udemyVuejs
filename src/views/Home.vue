<template>
  <div class="container p-3">
    ...
    <div class="d-flex flex-wrap">
      <div v-for="(device, ind) in deviceList" :key="ind" class="card m-3" style="width: 300px; background-color: #f6f6f6">
        <div class="card-body">
          <h5 class="card-title text-uppercase">{{ device.name }}</h5>
          <h6 class="card-subtitle text-muted">{{ device.description }}</h6>
        </div>

        <font-awesome-icon :style="{'color':device.color}" :icon="iconByDeviceType(device.deviceType)" class="mr-auto ml-auto mt-3 device-icon" />

        <div class="row pt-3 pb-3 pr-5 pl-5 mt-2">
...
export default {
  ...
  mounted() {
    DeviceService.getAllDevices().then((response) => {
      ...
      this.deviceList.forEach((d) => d.colors = this.colors.BLACK);
    });
  },
  methods: {
